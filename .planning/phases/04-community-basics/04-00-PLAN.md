...
must_haves:
  truths:
    ...
    - "8개 RED 테스트 스캐폴드 파일이 존재하고 `npm run test -- --run`이 실패(RED)한다"
  artifacts:
    ...
    - path: "src/lib/data/cafe-link.test.ts"
      provides: "COMM-03 RED 테스트 (카페 링크 URL 생성)"
    - path: "src/components/admin/SlaUtils.test.ts"
      provides: "COMM-04 RED 테스트 (SLA 24h 계산)"
...
Output: 2개 마이그레이션 파일 + `supabase db push` 완료 + 8개 테스트 스캐폴드 (모두 RED)
</objective>
...
`comment-actions` 모듈이 아직 없으므로 "Cannot find module" 에러로 RED 확인.
  ...
  <done>
    ...
    2. 8개 테스트 스캐폴드 파일이 존재함 (comment-actions, review-actions, cafe-link, SlaUtils, kapt, molit-presale, digest, topic-actions)
...
ls src/lib/data/cafe-link.test.ts
ls src/components/admin/SlaUtils.test.ts
...
7. 8개 테스트 스캐폴드 파일이 모두 RED 상태로 존재한다 (cafe-link.test.ts, SlaUtils.test.ts 포함)
</success_criteria>
