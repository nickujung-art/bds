...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/__tests__/member-tier.test.ts"
      ...
      min_lines: 30
    - path: "src/__tests__/naver-cafe.test.ts"
...
<task type="auto">
  ...
  <action>
    ...
    3. src/__tests__/naver-cafe.test.ts 작성:
    ```typescript
    /**
  ...
  <verify>
    <automated>npm run test -- --grep "member.tier|naver-cafe|kakao-channel|compare|TierBadge" 2>&1 | tail -20</automated>
  </verify>
  <acceptance_criteria>
    ...
    - src/__tests__/naver-cafe.test.ts 파일 존재
...
- grep -c "SECURITY DEFINER" supabase/migrations/20260512000001_phase8_gamification.sql 결과 >= 3
- grep -c "ENABLE ROW LEVEL SECURITY" supabase/migrations/20260512000002_phase8_kakao_channel.sql 결과 >= 1
- npm run test -- --grep "member.tier|naver-cafe|kakao-channel|compare" 실행 → RED (모듈 미존재 오류)
