...
tags: [naver-cafe, gemini-ner, nlp, cron, complex-matching, diff-02]
dependency_graph:
  requires:
...
tech_stack:
  added: []
  patterns:
    - Naver Search API (/v2/search/cafe) — KakaoAK 헤더 인증
...
key_files:
  ...
  modified:
    ...
    - src/__tests__/naver-cafe.test.ts (vi.resetModules() 추가 — Rule 1)
decisions:
...
# Phase 8 Plan 04: Naver 카페 NLP 단지 매칭 파이프라인 Summary

**One-liner:** Naver Search API + Gemini NER 파이프라인으로 카페 글을 수집하고 matchComplex() 3축 매칭으로 단지에 연결, 단지 상세 페이지에 카페 이야기 섹션 표시

---

## What Was Built

### Task 1: naver-cafe.ts 서비스 어댑터 (commit: 7a1cf6b)

**`src/services/naver-cafe.ts`**
- `searchCafePosts(query, size)`: Naver Search API `/v2/search/cafe` 호출 → `CafePost[]` 반환
  - `KakaoAK ${KAKAO_REST_API_KEY}` 헤더 인증
  - HTTP 오류 시 `Error("Naver cafe search HTTP {status}")` throw
...
**1. [Rule 1 - Bug] vi.resetModules() 추가 — naver-cafe.test.ts 모킹 이슈**
...
- **Files modified:** `src/__tests__/naver-cafe.test.ts`
- **Commit:** 7a1cf6b
...
**파일 존재 확인:**
- src/services/naver-cafe.ts: FOUND
...
**수락 기준 확인:**
- naver-cafe.test.ts 모든 테스트 PASS: YES (3/3)
